...
```mermaid
graph LR
    IoT[IoTセンサー\nゲームログ\nWebログ] --> KDS[Kinesis Data Streams\nシャード管理・カスタム処理\n完全リアルタイム]
    IoT --> KDF[Kinesis Data Firehose\nコード不要・自動配信\n準リアルタイム 数十秒〜数分]
    KDS -->|カスタムコンシューマー| Lambda[Lambda\nEC2]
    KDF -->|自動配信| S3[S3]
    KDF -->|自動配信| Redshift[Redshift]
    KDS --> KDA[Managed Service\nfor Apache Flink\nリアルタイム SQL 分析]
    style KDS fill:#ff9900,color:#fff
    style KDF fill:#232f3e,color:#fff
```
...
graph LR
    ...
    subgraph Store["② データレイク S3"]
        Raw[Raw データ\nS3]
    end
    subgraph Process["③ ETL処理"]
        Glue[AWS Glue\nスキーマ認識・変換]
        EMR[EMR\nHadoop/Spark\n大規模バッチ]
    end
    subgraph Analyze["④ 分析・クエリ"]
        Athena[Athena\nS3直接 SQL]
        RS[Redshift\nデータウェアハウス\nOLAP]
    end
    subgraph Viz["⑤ 可視化"]
